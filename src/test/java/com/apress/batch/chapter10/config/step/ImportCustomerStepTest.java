...
class ImportCustomerStepTest {
    ...
    @Test
    @DisplayName("CustomerItemUpdateReader 테스트")
    public void testCustomerItemReader() throws Exception {
        //given
        this.customerUpdateItemReader.open(new ExecutionContext());

        //when & then
        assertThat(this.customerUpdateItemReader.read()).isInstanceOf(
            CustomerAddressUpdate.class);
        assertThat(this.customerUpdateItemReader.read()).isInstanceOf(CustomerContactUpdate.class);
        assertThat(this.customerUpdateItemReader.read()).isInstanceOf(CustomerNameUpdate.class);
    }
}